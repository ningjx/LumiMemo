...
v2 已把 v1 中的悬置项全部定死。以下是**仅剩的四项**，每项都标注了截止点——到点必须定，否则会阻塞后续工作。**除这四项外，v2 中的所有设计选择均为结论，不再是待决项。**
...
2. **撤销**：桌面离开前台时，把上一步提过的那些便签清回普通档，并各自插回提升前的位置。
...
**锚点取自提升之前，不是"撤销那一刻的前台"。** 提升的时候就把每张便签当时的 **第一个算数的** z 序邻居（从 `GetWindow(GW_HWNDPREV)` 起往上走，隐藏窗口、桌面窗口、置顶窗口一律跳过）记下来，撤销时插回它后面——从哪儿来，回哪儿去。"当前前台"看着更顺手，实际是一张一直在变的牌，四条原因都实测踩过：
...
- **取锚点的时候，桌面窗口已经从底下被抬上来了——于是"便签上面那个"就是桌面自己。** 这条最隐蔽，也最致命：它让便签**每次都**掉到最底下，与走哪条入口、上一张是不是焦点窗口都无关。原因是**回调的时序**：`WINEVENT_OUTOFCONTEXT` 的事件是投递到注册线程的消息队列上的，等 `OnForegroundChanged` 真正跑起来，"显示桌面"早已把桌面窗口抬到普通窗口之上了（实测那一刻从便签往上问，第一个可见窗口正是 `Progman`）。把便签插到桌面后面——桌面本来就是普通档最底下那个——就是插到了**全屏幕最底下**。用户看到的正是"便签跑到最底下，再按显示桌面就跟着别的窗口一起消失/恢复"。**锚点的意义是"用户眼里盖住便签的那个窗口"，而桌面是背景板，从来不是"盖住"它的东西**，一律跳过。只有当便签上面本来就压着别的应用窗口时，问到的才是那个窗口——所以这条在"便签是普通档最前面一张"时才显形，也正是它被误判成"偶发"的原因。

前两条合起来，就是"Win+D 一切正常、点任务栏按钮却出问题"的全部来源——**按现象去分辨是哪条入口，反而会被带偏。** 第三条与入口无关，它才是「时好时坏」的来源——那串幽灵窗口每次排在哪一段都不一样，同一套操作做两遍可以一遍对一遍错。第四条同样与入口无关，但它是**必现**的：只要便签上面没有别的应用窗口，每次都会掉到底。

**撤销必须分两轮：先全部清标志，再全部归位。** 不能"清一张、归位一张"地穿插着做——给 A 归位时若 B 还挂在置顶档上，而 A 提升前恰好排在 B 下面（两张便签在屏幕上叠着），`SetWindowPos` 会连带把 A 也提拔进置顶档。两轮之间不返回消息循环，用户看不到中间态。
