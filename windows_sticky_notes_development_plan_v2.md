...
**"显示桌面"有两条入口，前台序列不一样。** 按 Win+D 时前台直接变成桌面窗口（`Progman`）；点任务栏右下角那个按钮时，前台先在**置顶档的** `Shell_TrayWnd`（任务栏）上停 **60～110 ms**（Win+D 只要 15～30 ms），之后才落到 `Progman`。经过任务栏这一次在前台序列里什么都不用做——它不等于桌面窗口，走的是撤销分支，而彼时提升记录还是空的，撤销等于空转。真正的坑在**撤销**那一步，见下面第二条例外。
...
**撤销那一步有两个必须特判的例外**，两个都是"只做清标志那一步、不插到锚点后面"：

1. **新前台是本进程的窗口**——说明这是系统在**还原**被"显示桌面"掀掉的那批窗口（它会先把管理器之类逐个还原，再把前台还给原来那个窗口），而不是用户点了别处。这时不动 z 序，否则便签会被插到管理器的后面，等系统把前台还给便签时，它就成了一张*名义上是前台、实际被盖住*的窗口（实测复现）。
2. **锚点自己就在置顶档**——把便签插到一个置顶窗口后面，`SetWindowPos` 会**连带把便签也提拔进置顶档**。点任务栏那个按钮时踩的正是这里：撤销那一刻前台是置顶的 `Shell_TrayWnd`，于是清掉临时置顶 **3 ms 后**便签又变回 `WS_EX_TOPMOST`。更糟的是这个错误的置顶**再也撤不掉**——提升那一步会跳过已经置顶的窗口，而撤销那一步只处理自己提升过的那些。这时只做第一步就够了：`HWND_NOTOPMOST` 的语义本来就包含"排到所有非置顶窗口之上"，比插到一个置顶窗口后面更接近我们想要的落点。

第二条从现象上看是"点了显示桌面按钮就再也不消失"，而 Win+D 那条路完全正常——因为两条入口的前台序列不同（见上）。**排查时先分辨是哪条入口**，别把它当成 Win+D 的回归。
...
> "显示桌面开始了"；但**撤销**那一步会把那个置顶的任务栏当成锚点，必须专门跳过——理由见 §13.6
> 的第二条例外。这也是"Win+D 一切正常、点按钮却出问题"的来源。
