...
v2 已把 v1 中的悬置项全部定死。以下是**仅剩的六项**，每项都标注了截止点——到点必须定，否则会阻塞后续工作。**除这六项外，v2 中的所有设计选择均为结论，不再是待决项。**
...
| D-6 | 「稍后再说」这一档做不做：§11.4 的第三个按钮，与 §15.2 的 `外部已修改 · 点击处理` 状态条 | **要做**，但体量接近一个独立工作项（可点命令 + 便签状态条 + 「不要自动重复弹窗」的抑制规则），因此不在本轮 | 一期收尾前 |

**D-4 不是设计问题，是时间问题**——启动当天查一下最新的 LTS 版本号即可，列在这里只是提醒不要在实现中途升级目标框架。

**D-5 是某一轮才发现的**：`AppSettings.MaxOpenWindows` 默认 50，字段表把处置方式指向 §13.9，但 §13.9 只讲了「不为每一张便签创建窗口」的实现约束，没有任何一句说超限时该拦在哪一步、该提示什么、是拒绝新开还是先关最旧的。目前实现里**没有任何一处读它**（不阻塞，先按 50 这个值不动）。

**D-6 是文件监听这一轮落下的缺口**：§11.4 的冲突对话框实现成了两档（重新加载／覆盖外部版本），第三档「稍后再说」与它依赖的 §15.2 状态条 `外部已修改 · 点击处理` 都没做。眼下**关掉对话框就等于是稍后再说**——两侧都不动，只记一行日志。缺口在于**没有第二次提醒**：用户当时选了等一等，之后不会再有东西告诉他这件事还悬着（不自动重弹是 §11.4 原文就要求的，所以这里缺的是那个「随时可以点开重新处理」的状态条，不是重弹）。
...
**一、`LoadAllAsync` 改成了按 id 合并，不再是 `Clear()` + 逐条 `Add`。** 这一改动同时修掉一个既有的数据丢失缺陷，也因此成了 §10.4 溢出恢复与 §11.4 静默重载的公共实现。

原来的写法是清空 Store 再重建。而窗口绑的是 `Note` **实例**（`NoteViewModel.Note` 持有的是同一个引用），重扫之后 Store 里换成了新实例、窗口还握着旧的：用户在窗口里改的字既不进 Store 也存不下去（`SaveNoteAsync` 按 id 从 Store 取到的是新实例）。**托盘的「重新加载全部便签」在便签开着的时候会丢掉改动。**

现在的合并规则（`NoteService.LoadAllAsync`）：

```text
同 id     → 把字段搬进 Store 里**已有的那个实例**（Note.CopyFrom；Tags 原地 Clear/AddRange）
磁盘有、Store 没有 → 新增（layout 里 IsOpen 为真才开窗，见 §17.1 第 11 步）
Store 有、磁盘没有 → 删除
两边都有且相同     → 不动（避免无谓的刷新）
```

**搬字段而不是换实例**是这条规则的全部要点：引用不变，开着的窗口才不会悬空。

**二、`NoteStore` 加了一个 `TryGetByPath(string path)`**（内部一张 `OrdinalIgnoreCase` 的路径 → id 索引，在 `Add`/`Update`/`Remove`/`Clear` 里同步维护）。外部把文件删掉之后，磁盘上已经没有那个文件了，只能靠路径把内存里对应的便签找回来。
...
**上面两层在实现里合并成了一层，第一层的 3 秒写入抑制窗口被去掉了。** 判据只有一个：**磁盘此刻的字节，是否等于本程序上次读或写这个文件时的字节**。

去掉那个窗口是因为它会对准本工作项要防的那件事开火。两个反例：

```text
t=0     本程序保存，窗口开到 t=3
t=1     用户在 VS Code 里改了同一个文件并保存
t=1.6   watcher 事件到达 → 命中窗口 → 丢弃
t=3.5   用户在便签窗口里又敲了一个字，自动保存
        → 内存里那份（不含外部改动）被写回磁盘，外部那一版永久消失
```

```text
t=0     本程序保存，窗口开到 t=3
t=0.5   用户执行 git checkout 切分支，一批文件被重写
        → 整次分支切换的事件全被一个窗口吞掉，内存与磁盘静默分家
```

窗口越短越挡不住自己的写、越长越吞外部改动，**这个权衡本身没有好答案**——所以不权衡，改用「比字节」这一条与时间无关的判据。

**第二层（每路径 8 条 / 5 秒的有界 hash 集合）也不必存在**，因为本实现**每个事件都重新读盘**：比较的是当前磁盘字节，不是事件携带的历史哈希，于是迟到的重复通知只会再比一次当前状态，结论天然正确。v1「只记最后一个 hash」的那个漏洞在这里不成立，原因不是集合变大了，而是压根不靠历史事件里的哈希。有界集合的全部意义是「不读盘也能认出自己写的」——我们不省这一次几 KB 的读。

**归一化**照本节最后一段说的做：比的是**字节序列的 `SHA256`**，读盘时对读出的原始字节取哈希，写盘时对将要写出的字节取哈希。于是 BOM 与行尾的差异都不会造成误判，编码探测的结果也不必参与判断。
...
**流程**：`FileSystemWatcherAdapter.OnError` 把 `FileWatchChangeKind.Error` 报出去 → `FileWatchService` 记一条 Warning、开 1 秒的合并窗口 → 到期后 `_watcher.Stop(); _watcher.Start();` 重建监听器，再走一次全量重扫（`ManagerViewModel.ApplyFullRescanAsync` → `NoteService.LoadAllAsync`）。差异比对就是 §10.4 上面那四种情况，落在 `NoteService.LoadAllAsync` 里按 id 合并（见 §9 那节末的实现说明）。

**重建必须在重扫之前**：溢出之后监听器是坏的，重扫期间发生的新改动只有重建过的那个实例才收得到。顺序反了的话，「重扫读到的那一版之后、新监听器生效之前」这个窗口里落地的文件会被彻底漏掉——而溢出恢复的全部意义就是补回丢失的变更。

**重扫进行中，普通事件不重置那个 1 秒窗口**。溢出之后往往还跟着一大批刚落地的文件事件，它们要是也去重开窗口，重扫就会被一直推迟，而重扫是「重新开始监听」的前置——推迟期间监听器一直是坏的。这些事件本身不会丢：它们照常进批次，重扫补得回内容、补不回「刚才谁动过它」。

**溢出事件不带路径**：`Error` 里那句 `_paths.NotesFolder` 只是为了让 `FileWatchChange.Path` 有个值，「谁动过」这条信息恰恰是丢掉的那一部分。
...
**上面三条里只有第 2 条落地了**：托盘的「重新加载全部便签」（`ManagerViewModel.ReloadAllAsync`）本轮接到了新语义上，走的就是 §10.4 那套全量重扫 + 按 id 合并；溢出恢复用的也是它。

**第 1 条（启动时用 `DriveInfo` 检测网络盘并提示）与第 3 条（可选的轮询兜底开关）本轮不做**。它们要的是设置页上的一处新 UI 与一个新设置项，而手动兜底这条路本来就在——网络盘用户至少有一个逃生出口，不会卡在没有出路的状态。等设置页那一轮一起做。

**读盘走 `Task.Run`**：`ReloadAsync` 内部是同步的 `File.ReadAllBytes`，网络盘上一次读可能好几秒，不能落在 UI 线程上（§3.4 规则 T6）。
...
本节的三处设计在实现时做了调整，**§17.1 第 10 步一直引用的「见 §10 的偏离说明」就是这一段**。

**落点**：`LumiMemo.Infrastructure/Watching/FileSystemWatcherAdapter.cs` 只做「把 Windows 事件翻译成 `FileWatchChange`」，不做去抖、不读盘；去抖、读盘、封送在 `LumiMemo.App/Services/FileWatchService.cs`；「这批变更落到界面与内存上该怎么办」在 `ManagerViewModel.ApplyExternalChangesAsync`（编排，因为 Core 不能弹 UI），判定本身在 `NoteService.ApplyExternalChange`（业务，因为 Core 才认识 `NoteStore` 与索引）。四层各管一段，没有哪一层同时认识文件和窗口。

**一、自写抑制去掉了时间窗口**（对 §10.3 的实质偏离，理由见那里的实现说明）。判据只剩一条：**磁盘此刻的字节是否等于本程序上次读或写这个文件时的字节**。那份基线本来就在（`MarkdownNoteRepository` 的 `_states[path].ContentHash`，读入、补 id 回写、保存成功三处都在更新），它既当自写凭据，又当 §11.4 三路比较里的「基线」。**代价**是每个事件都要读一次盘——便签只有几 KB，一次读换掉一整类误判，划算。

**二、去抖改成一个全局合并窗口**（对 §10.2 的偏离），而不是按路径一棵 `CancellationTokenSource`：一个待处理路径集合 + 一个 `IUiTimer`，每个事件重开这个 300ms 的窗口，到期后一次性处理整批。两条理由：其一，`Task.Delay` 在无消息泵的测试进程里无法确定性驱动，而本仓库为「连打 5 个字符只写一次盘」专门立了 `IUiTimerFactory` 端口；其二，单个合并窗口天然把批量操作（git 切分支、同步盘刷一批文件）合成一次处理，与 §10.4 自己规定的「1 秒合并窗口」同一个道理。**代价**：持续的写入流会把处理一直推迟到下一次静默期。批量场景下这正是想要的，但「另一个进程每隔 100ms 就碰一下同一个文件」这种极端情况下，变更会一直攒着不落地。

**三、监听器只报扫描器也会收的文件**。过滤交给 `INoteRepository.IsNoteFile`，而不是在监听器里另写一遍——两处判据一旦分家，用户改一下 Obsidian 的配置就会在便签列表里多出一张重启后又不存在的幽灵便签。`Filter` 与 `NotifyFilter` 按 §10.1 原样设置，`InternalBufferSize` 64KB、五个事件全订。

**换笔记目录需要重启程序**（§8.6 同）。`Start()` 时若笔记目录尚未选定（首次运行的向导还没走完），是空操作加一条 Warning——那不是错误，不该让程序启动不了。笔记目录后来被选定时需要有人再调一次 `Start()`，目前那意味着重启。
...
**一、判定表中间两行的条件与解释写反了**。原文照留（它记的是当时的想法），实现按**意图**走：

| 原文那一行 | 原文的解释 | 实现里的意思 |
|---|---|---|
| 磁盘 == 基线，本地 != 基线 | 「用户在外部编辑器里改了，但本地没改」 | 条件说磁盘没变、解释却说外部改了，互斥。**按解释实现**：磁盘变了、本地没变 → 静默重载 |
| 磁盘 != 基线，本地 == 基线 | 「本地改了（但还没保存），外部没改」 | 同上互斥。**按条件实现**：本地没改 → 静默接受，更新基线 |

**二、三个输入在代码里各是什么**：

| 文档 | 代码 |
|---|---|
| 磁盘 | `MarkdownNoteRepository.ReloadAsync` 本次读到的便签 + `NoteFileSync.DiskChanged` |
| 基线 | `_states[path].ContentHash`——就是 §10.3 里那份自写凭据 |
| 本地 | `NoteStore` 里那张便签，以及「它有没有还没落盘的改动」 |

判定过程在 `NoteService.ApplyExternalChange`，结论以 `ExternalChangeResult` 返回（`None` / `Reloaded` / `Created` / `Deleted` / `Conflict`）。**Core 只回答「内存变成了什么样」，不决定界面怎么办**——开窗、关窗、弹对话框都需要窗口层与对话框，而 Core 两样都不认识（§3.1）。

**三、「有没有未落盘的改动」由 `NoteService` 自己维护**（一个 `HashSet<Guid>`）：`ApplyLocalEdit` / `ApplyColorEdit` / `ApplyTagsEdit` 置位，保存成功与静默重载后清位。这是**唯一**拿得到这个信号的地方——`NoteViewModel.IsDirty` 全仓库无人写入（只有一条测试在断言它初始为 `false`），`AutoSaveService._pending` 在 App 层且语义是「有一轮排队中」，两者都不能用。

**副作用要说清**：把字打完再删回原样，标志仍然立着 → 偏向**弹框问用户**而不是静默覆盖。这是有意的：多问一次的代价是用户点一下，不问的代价可能是他刚打的字没了。

**四、对话框是两档**，复用现成的 `IDialogService.ChooseAsync`（标题、正文、两个按钮）：

- **重新加载（放弃我的改动）** → `NoteService.ResolveConflictByReload`，把磁盘版搬进内存里**那个实例**（引用不变，开着的那扇窗不会悬空），再刷新窗口。
- **覆盖外部版本（保留我的改动）** → 先 `BackupConflictCopyAsync` 把磁盘那一版另存为 `{无扩展名}.conflict-{yyyyMMdd-HHmmss}.md`，**成功了才**写回去。备份写不出来（磁盘满、没有权限）时抛异常，管理器接住并提示用户「磁盘上那一份没有被改动，可以稍后再试」——那一条路径上绝不能覆盖，否则外部那一版就永久没了。

**五、「稍后再说」这一档本轮不做**（第三档按钮与 §15.2 的 `外部已修改 · 点击处理` 状态条，见 §0.4 的 `D-6`）。**眼下关掉对话框就等于是稍后再说**：两侧都不动，只记一条 Information 日志。用户回头发现两张内容不一致时，那是唯一能说明「当时问过、他选了等一等」的线索。少了那个状态条意味着**没有第二次提醒**——不自动重弹是有意的（原文也这么要求），但用户也可能就此忘掉。

**六、`.conflict-` 备份落在同一个目录里**（换个目录用户就找不到它了）。**代价**：它下次启动扫描时会作为一张新便签出现——文件里没有 id，会被补给一个。这是知情的取舍：比悄悄丢掉一边强。用户确认过之后自己删掉即可。
...
4. **第 5 步不在启动序列里**：`*.lumitmp` 的清理在 `MarkdownNoteRepository` 扫描时顺手做掉，不必单独走一趟目录。第 9 步（全局热键）、第 12 步（开机自启动）**整体推迟**，见 §17.6。**第 10 步（`FileSystemWatcher`）已落地**，就是本节下面这一条。
...
**第 10 步已落地**（`FileWatchService.Start()`，由 `StartupSequence.RunAsync` 调用）。位置有三处讲究，都不是随手放的：

1. **必须排在第 6 步那次扫描之后。** 反过来的话，扫描期间到达的事件会去比对一份还没建好的 `NoteStore`；更要命的是扫描本身要把「上次同步的字节」基线填进仓储，而那是自写抑制（§10.3）的判据——基线没填之前，每一个事件看起来都像外部改动。
2. **紧跟在扫描后面，而不是排到整个启动序列末尾。** 从扫描结束到监听开始这一段空档里，用户改的 `.md` 没有任何人会看见（监听器还没跑，扫描也已经过去了）。后面那几步（恢复上次开着的便签）可能要几百毫秒，不该白白算进这段空档。
3. **关停时它是第一件事**（`ShutdownAndWait` 的 `Task.Run` 体里紧跟着 `AutoSaveService.Dispose()`）。再往后每多待一秒，就多一次「退出到一半、外部又改了文件」的机会，而那时冒出来的窗口是在进程正在死的当口创建的。

**换笔记目录需要重启程序**（与 §8.6 一致）：`Start()` 时笔记目录若尚未选定就是空操作 + 一条 Warning，目录后来被选定了也不会自动开始监听。
...
上面 `[FileSystemWatcher]` 那一组拆成了两层，因为它们的可测性完全不同：

**真的 Windows `FileSystemWatcher` 只由集成测试覆盖**——「Windows 到底会不会报这个事件、报的是哪一种」替身替不掉，而那正是 `FileSystemWatcherAdapter` 的全部内容（把 `Renamed` 与「删除 + 新建」区分开、把不该收的路径挡在外面）。落在 `LumiMemo.Integration.Tests/Watching/FileSystemWatcherAdapterTests.cs`：五种事件各报各的、子目录里的 `.md` 也报、`.obsidian` 与 `attachments` 与 `.txt` 不报、`Start()` 幂等、`Stop()` 之后不再上报而再 `Start()` 又能收到。

**「报告之后的处理」由单元测试覆盖**（`LumiMemo.App.Tests/Services/FileWatchServiceTests.cs`）：去抖合并、同路径只读一次盘、`DiskChanged` 为假的不进管理器、重命名时新路径排在旧路径前面、一条读不出来其余照常、溢出重建并重扫、溢出后普通事件不推迟重扫、`Dispose` 之后不再响应。时间由 `RecordingUiTimerFactory` 手工推动，「事件什么时候到」由 `FakeFileWatcher` 决定——两者都不真等。

`WaitUntilAsync` 已落在 `LumiMemo.Integration.Tests/Waiting.cs`（默认超时 5 秒，轮询间隔 25 毫秒，超时抛 `TimeoutException`）。

**「什么都没发生」这类断言需要一个顺序上的界**。直接等一段固定时间再断言「列表是空的」最容易写、也最容易飘红：机器一慢就假绿（事件迟到），机器一快就假红。做法是造一条**对照事件**（在笔记目录里写一个普通便签文件并等它到达）——同一个 `FileSystemWatcher` 实例的事件是有序的，对照事件既然到了，那在它之前动过的那个文件如果会被上报，也早该到了。
