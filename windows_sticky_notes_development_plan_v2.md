...
**锚点取自提升之前，不是"撤销那一刻的前台"。** 提升的时候就把每张便签当时的 z 序邻居（`GetWindow(GW_HWNDPREV)`，也就是当时盖住它的那个窗口）记下来，撤销时插回它后面——从哪儿来，回哪儿去。"当前前台"看着更顺手，实际是一张一直在变的牌，两条原因都实测踩过：
...
这两条合起来，就是"Win+D 一切正常、点任务栏按钮却出问题"的全部来源——**按现象去分辨是哪条入口，反而会被带偏。**
...
**锚点不可用时就只做前一轮。** 三种情况：锚点为空（便签提升前就在 z 序最顶，而 `HWND_NOTOPMOST` 给它的位置正是原位）、锚点已失效（提升与撤销之间隔着整个"显示桌面"，那个窗口可能已经被销毁，句柄还可能被复用）、锚点自己就在置顶档（插到置顶窗口后面会连带提拔，见上）。后两种都在实测里出现过。
